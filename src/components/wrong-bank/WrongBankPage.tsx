...
export default function WrongBankPage() {
  ...
  const pending = wordBank.filter((w) => w.isWrong).length;

  const onStart = () => {
    const q = build();
    if (!q.length) {
      alert('暂无可复习的错词～先去查单词答错几个试试！');
      return;
    }
    startReview(q);
  };
  ...
  const onSingleTest = (word: string) => {
    const one = useAppStore.getState().wordBank.filter((w) => w.word === word && w.isWrong);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">单词库</h1>
          <p className="text-sm text-ink-muted mt-0.5">
            查过的词自动入库，答错的标记为错词；连续答对 N 次后自动攻克（默认 3，可在设置中调整）
          </p>
        </div>
        <button
          type="button"
        ...
        >
          <PlayCircle size={20} /> 复习错词（{pending}）
        </button>
      </div>
